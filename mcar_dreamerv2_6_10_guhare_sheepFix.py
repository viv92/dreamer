...
class DreamerV2(nn.Module):
    ...
    def train(self):
        ...
        state_values = self.critic_V( im_states.detach(), im_beliefs.detach() ) # v[t+1 : t+H+1]
        critic_target = lambda_returns.clone().detach()
        loss_critic = F.mse_loss(state_values[:-1] * torch.pow(discounts_cumprod, 0.5), critic_target * torch.pow(discounts_cumprod, 0.5))
        ...
        advantage = (lambda_returns - state_values[:-1]).detach() # advantage[t+1 : t+H]
...
if __name__ == '__main__':
    ...
    rho = 1 # 0.75 # 0.25 # 0 # used for weighing actor dynamics loss and actor reinforce loss
    eta = 1e-3 # used for weighing entropy regulaization in actor loss
    df = 0.995
    df_loss_scale = 1
    obs_loss_scale = 1 # 1e-1
    reward_intrinsic_scale = 1 # 1e-1
    ...
    action_repeat = 1 # 2
    explore_minLimit, explore_maxLimit = 0.1, 0.9 # 0.05, 0.4
    explore_mult = 1
    explore_decay = (explore_maxLimit - explore_minLimit) / (delay_ep * 2) # 50 # 100
    max_ep_steps = 200 # this isn't used for anything
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            # terminal reward to overshadow curiosity reward
            # also we have only one (desirable) terminal state for mcar 
            if terminated:
        ...
        ## episode ended
        # train agent
        if ((ep+1) % train_episode == 0) and (agent.replay_buffer.n_items > sample_seq_len * 2):
