### Program implementing DreamerV2 on CartPole-v1 environment
...
# This implementation is same as dreamerv2_5_1_12_3_cu.py but with internal belief (input to rssm = prev_belief + prev_action + curr_obs; input to all other models = state)
...
# RSSM used for both Representation model p(s_t | h_t-1, a_t-1, o_t) and Transition model q(s_t | h_t-1, a_t-1)
class RSSM(nn.Module):
    def __init__(self, in_dim, o_dim, belief_dim, h_dim, out_dim, batch_size, device):
        super().__init__()
        self.fc_embed = nn.Linear(in_dim, belief_dim) # layer to embed input (a_t)

        # init deterministic recurrent net (shared between p and q models)
        self.gru_cell = nn.GRUCell(belief_dim, belief_dim) # belief h_t = f(h_t-1, a_t-1)
...
class StochasticNet_Gaussian(nn.Module):
    ...
    def forward(self, state):
        ...
        # logstd = self.fc3_std(h).clip(-20, 2)
...
class Actor(nn.Module):
    ...
    def policy_dist(self, state):
        logits = self.forward(state)
        dis = OneHotCategorical(logits=logits)
        return dis

    # returns policy log_prob
    def policy_logprob(self, state, y):
        policy_dis = self.policy_dist(state)
...
class DreamerV2(nn.Module):
    def __init__(self, o_dim, s_dim, a_dim, belief_dim, h_dim, seq_len, imagination_horizon, df, buf_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, tau, rho, eta, max_ep_steps, frac, device):
        super().__init__()
        self.actor = Actor(s_dim, a_dim, h_dim).to(device)
        ...
        self.rssm = RSSM(a_dim, o_dim, belief_dim, h_dim, s_dim, batch_size, device).to(device)
        self.df_model = StochasticNet_Bernoulli(s_dim, h_dim, 1).to(device)
        self.reward_model = StochasticNet_Gaussian(s_dim, h_dim, 1).to(device)
        self.observation_model = StochasticNet_Gaussian(s_dim, h_dim, o_dim).to(device)
    ...
    def get_action(self, state):
        policy = self.actor.policy_dist(state)
    ...
    def train(self):
        ...
        for t in range(self.seq_len-1):
            ...
            # log prob q(r_t | s_t)
            lp_reward = self.reward_model.log_prob(curr_state, prev_reward) * (1. - prev_prev_done) # skip reward loss when curr state is first state of new episode
            # print('lp_reward.shape: ', lp_reward.shape)
            lp_reward = lp_reward.sum(dim=1).mean()

            # log prob q(o_t | s_t)
            lp_obs = self.observation_model.log_prob(curr_state, curr_obs)
            # print('lp_obs.shape: ', lp_obs.shape)
            lp_obs = lp_obs.sum(dim=1).mean()

            # log prob q(df_t | s_t)
            df_target = 1. - prev_done
            lp_df = self.df_model.log_prob(curr_state, df_target)
        ...
        for tau in range(self.imagination_horizon):
            im_curr_reward = self.reward_model.sample(im_curr_state) # reward obtained on transitioning into curr_state from (prev_state, prev_action)
            im_curr_df = self.df_model.sample(im_curr_state) # whether curr_state is terminal state or not ( so df[t] * s[t] )
            im_curr_df = im_curr_df * self.df
            # im_curr_df = self.df_model.sample(im_curr_state, im_curr_belief) * self.df

            im_curr_action = self.get_action(im_curr_state)
            im_curr_pi = self.actor.policy_dist(im_curr_state)
            im_curr_log_pi = im_curr_pi.log_prob(torch.round(im_curr_action.detach()))
            im_curr_entropy_pi = im_curr_pi.entropy()
            # rollout step
            rssm_input = im_curr_action
            im_next_state, im_next_belief = self.rssm.sample(im_curr_belief, rssm_input)
            # store required values
            state_values.append(self.critic_V( im_curr_state.clone().detach() )) # [0 : H-1]
            state_values_target.append( self.target_critic_V(im_next_state).detach() ) # [1 : H]
...
if __name__ == '__main__':
    ...
    rho = .95 # used for weighing actor dynamics loss and actor reinforce loss
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            rssm_input = prev_action.to(device)
...
hyperparam_dict['algo'] = 'dreamerV2_5_1_12_3_internalBelief'
