...
class RSSM(nn.Module):
    def __init__(self, in_dim, o_dim, belief_dim, h_dim, out_dim, batch_size, device):
        super().__init__()
        self.fc_embed = nn.Linear(in_dim, belief_dim) # layer to embed input (s_t, a_t) # TODO: maybe add more layers for fc_embed
...
class StochasticNet_Gaussian(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc3_std(h).clip(minClip, 2)
...
class DreamerV2(nn.Module):
    def __init__(self, o_dim, s_dim, a_dim, belief_dim, h_dim, sample_seq_len, ep_seq_len, imagination_horizon, df, buf_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, tau, rho, eta, frac, device):
        super().__init__()
    ...
    def train(self):
        ...
        done = done.float().clip(0, 1) # [time, batch, 1]

        ## prepare sampling indices based on done targets and terminal frac

        # get terminal state indices
        flip_idx = done.argmax(dim=0).squeeze(-1) # [batch]

        # range of sample indices containing terminal state
        low_flip = torch.clip(flip_idx - self.sample_seq_len + 1, min = 0)
        high_flip = torch.clip(flip_idx, max = self.ep_seq_len - self.sample_seq_len)

        # all sample indices
        low_all = torch.zeros_like(flip_idx)
        high_all = torch.full_like(flip_idx, self.ep_seq_len - self.sample_seq_len)

        # create sample mask based on frac 
        mask = torch.rand_like(flip_idx.float()) < self.frac

        # select sample index range based on mask 
        low = torch.where(mask, low_flip, low_all)
        high = torch.where(mask, high_flip, high_all)

        # get the sample start idx 
        rand = torch.rand_like(low.float())
        start_time_idx = low + ((high - low + 1).float() * rand).floor().long()
        ...
        # list tensors to store states and beliefs obtained from representation model - used for dynamics training rollout and imagination rollout
        # prev_state_list = []
        # prev_belief_list = []
        # prev_action_list = []
        # prev_reward_list = []
        ...
        prev_state = torch.zeros(self.batch_size, self.s_dim).to(self.device) # NOTE that all zeros is invalid one-hot state
        ...
        #     curr_belief, state_prior, curr_state, logits_prior, logits_posterior = self.rssm.sample(prev_belief, rssm_input, curr_obs)
        ...
        # NOTE that model dynamics training targets should be consistent - a particular state should not get different reward targets or different df targets on different train samples - this also implies that all terminal states should be true absorbing states
        for i in range(self.sample_seq_len):
            ...
            curr_action = action[time_idx, batch_idx]
            curr_reward = reward[time_idx, batch_idx]
            curr_done = done[time_idx, batch_idx] 
            ...
            rssm_input = torch.cat((prev_state, prev_action), dim=1)
            curr_belief, state_prior, state_posterior, logits_prior, logits_posterior = self.rssm.sample(prev_belief, rssm_input, curr_obs)

            im_state_list.append(state_posterior.clone().detach()) # s_t - store state for imagination rollout
            im_belief_list.append(curr_belief.clone().detach()) # h_t - store belief for imagination rollout

            # log prob q(r_t | s_t) # NOTE maybe we want if(i>0) here
            if i > 0:
                lp_reward = self.reward_model.log_prob(state_posterior, curr_belief, prev_reward)
                lp_reward = lp_reward.mean()
            else:
                lp_reward = 0

            # log prob q(o_t | s_t)
            lp_obs = self.observation_model.log_prob(state_posterior, curr_belief, curr_obs)
            lp_obs = lp_obs.mean() # NOTE: sum of logprobs implies product of probs. Think we should just take mean (no sum)

            # log prob q(df_t | s_t)
            df_target = 1. - curr_done
            lp_df = self.df_model.log_prob(state_posterior, curr_belief, df_target)
            lp_df = lp_df.mean()
            ...
            # next time step
            prev_state = state_posterior.detach()
            prev_belief = curr_belief.detach()
            prev_action = curr_action
            prev_reward = curr_reward
        ...
        im_curr_belief = torch.flatten(im_belief_list, start_dim=0, end_dim=1) # avoid the zero start state

        # flatten time and batch dimension into one - for parallel rollouts
        im_curr_state = torch.flatten(im_state_list, start_dim=0, end_dim=1) # avoid the zero start state
        ...
        for tau in range(self.imagination_horizon):
            ...
            im_next_belief, im_next_state, _, _, _ = self.rssm.sample(im_curr_belief, rssm_input)
...
if __name__ == '__main__':
    ...
    belief_dim = 128 # 200
    lr_actor = 1 * (4e-5) # 4e-5
    lr_critic = 1 * (1e-4) # 1e-4
    lr_model = 1 * (2e-4) # 2e-4
    sample_seq_len = 64 # 32 # 16 # 50 # length of contiguous sequence sampled from replay buffer (when training)
    imagination_horizon = 16 # 8 # 15 # length of imagined rollouts using the learnt dynamics model (when behaviour learning)
    vib_beta = 0.1 # 1 # beta - tradeoff hyperparam in vib objective
    reward_intrinsic_scale = 10
    delay_ep = 20
    ...
    rho = 0.25 # 1. # 0. # 0.75 # 0.5 #.95 # used for weighing actor dynamics loss and actor reinforce loss
    eta = 1e-3 # used for weighing entropy regulaization in actor loss
    df = 0.995
    minClip, maxClip = -2, 2
    STD = 0.1 # 1
    frac = 0.25 # fraction of sampled episodes containing terminal state transitions
    batch_size = 64
    replay_buffer_size = 10**3
    num_episodes = 300 # 100
    random_seed = 1010
    record_episode = num_episodes // 5
    init_random_episodes = 0 # 10 # 10 # 50
    num_train_calls = 10 # 30 # 1 # 100
    train_episode = 1 # 10
    action_repeat = 2
    ...
    hyperparam_dict['algo'] = 'dreamerV2_6_10_df1_noHistory_noRewNorm_ICMDream'
    ...
    # hyperparam_dict['xploreMin'] = str(explore_maxLimit)
    hyperparam_dict['frac'] = str(frac)
    hyperparam_dict['minClip'] = str(minClip)
    hyperparam_dict['maxClip'] = str(maxClip)
    hyperparam_dict['intRewScale'] = str(reward_intrinsic_scale)
    # hyperparam_dict['std'] = str(STD)
    hyperparam_dict['epDelay'] = str(delay_ep)
    ...
    # init Dreamer agent
    agent = DreamerV2(o_dim, s_dim, a_dim, belief_dim, h_dim, sample_seq_len, max_time_steps+1, imagination_horizon, df, replay_buffer_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, tau, rho, eta, frac, device)
    ...
    for ep in range(init_random_episodes):
        ...
        while done == 0:
            ...
            # truncate if max_time_steps reached 
            if len(ep_trace) == (max_time_steps - 1):
                done = 2
            ...
            if done:
                ...
                while len(ep_trace) < (max_time_steps + 1):
                    ...
                    if done == 1: # terminated
                        terminal_reward = reward_normalized * 0 # terminal state is truly absorbing
                    else:
                        terminal_reward = reward_normalized * 0 # terminal state is truly absorbing
    ...
    # epsilon_schedule[:50] = np.linspace(1., explore_minLimit, 50)
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        with torch.no_grad():
            ...
            while done == 0:
                ...
                reward_normalized = reward_extrinsic + reward_intrinsic
                ...
                ep_return += (df ** ep_steps) * reward_normalized
                ep_return_extrinsic += (df ** ep_steps) * reward_extrinsic 
                ep_steps += 1
                interaction_steps += 1
        ...
        ## episode ended
        # train agent
        if ((ep+1) % train_episode == 0) and (agent.replay_buffer.n_items > 0):
        ...
        if ep % (num_episodes//10) == 0:
            print(f'ep:{ep} \t ep_return:{ep_return:.3f} \t ep_return_extrinsic:{ep_return_extrinsic:.3f}')

        # save episode recording 
        if (ep+1) % record_episode == 0:
            # print(f'recorded ep:{ep} done:{done} terminal_reward:{reward} terminal_reward_normalized:{reward_normalized}')
...
ax[0,1].legend()
ax[0,1].set_title('dyn:{:.2f}'.format(loss_dynamics_moving_mean[-1]))
