### Program implementing Dreamer on MountainCarContinuous-v0 environment
...
class Dreamer(nn.Module):
    def __init__(self, o_dim, s_dim, a_dim, belief_dim, h_dim, max_action, seq_len, imagination_horizon, df, buf_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, frac, tau, device):
        super().__init__()
    ...
    def calculate_lambda_return(self, rewards, state_values_target):
        ...
        for t in range(self.imagination_horizon-2, -1, -1):
    ...
    def calculate_lambda_return_yuku(self, imged_reward, value_pred, bootstrap, discount=0.99, lambda_=0.95):
        # Setting lambda=1 gives a discounted Monte Carlo return.
        # Setting lambda=0 gives a fixed 1-step return.
        imged_reward = imged_reward[:-1] # [0 : H-2]
        next_values = value_pred[1:] # [1 : H-1]
    ...
    def imagine_ahead(self, prev_state, prev_belief, policy, transition_model, planning_horizon=12):
        ...
        for t in range(T - 1):
            ...
            actions = self.get_action(_state.detach(), beliefs[t].detach())
            # Compute belief (deterministic hidden state)
            rssm_input = torch.cat((_state, actions), dim=1)
            prior_means[t + 1], prior_std_devs[t + 1], beliefs[t + 1] = self.rssm.forward(beliefs[t], rssm_input)
    ...
    def yuku_transition_model(
        ...
        for t in range(T - 1):
            ...
            # Compute belief (deterministic hidden state)
            rssm_input = torch.cat((_state, actions[t]), dim=1)
            prior_means[t + 1], prior_std_devs[t + 1], beliefs[t + 1] = self.rssm.forward(beliefs[t], rssm_input)
            prior_states[t + 1] = prior_means[t + 1] + prior_std_devs[t + 1] * torch.randn_like(prior_means[t + 1])
            posterior_means[t + 1], posterior_std_devs[t + 1], beliefs[t + 1] = self.rssm.forward(beliefs[t], rssm_input, observations[t])
    ...
    def train(self):
        ...
        posterior_states, \
        posterior_means, \
        posterior_std_devs = self.yuku_transition_model(prev_state, action[:-1], prev_belief, observation[1:], nonterminals[:-1])

        # start training steps
        for t in range(len(beliefs)):
            curr_state, curr_belief = posterior_states[t], beliefs[t]
            rssm_input = torch.cat((curr_state, action[t]), dim=1)
            ...
            # log prob q(r_t | s_t)
            lp_reward = self.reward_model.log_prob(curr_state, curr_belief, reward[t]).sum(dim=1).mean()

            # log prob q(o_t | s_t)
            lp_obs = self.observation_model.log_prob(curr_state, curr_belief, observation[t+1]).sum(dim=1).mean()

            div = tdist.kl.kl_divergence(tdist.Normal(posterior_means[t], posterior_std_devs[t]), tdist.Normal(prior_means[t], prior_std_devs[t]))
            # kl_div = torch.max(div, 3).mean()
            ...
            kl_div = torch.max(div, free_nats)
            kl_div = kl_div.mean()
        ...
        imagination_traj = self.imagine_ahead(state_list, belief_list, self.actor, self.rssm, self.imagination_horizon)
        imged_beliefs, imged_prior_states, imged_prior_means, imged_prior_std_devs = imagination_traj

        value_pred = [] # state values obtained from critic [v(s_0) : v(s_H-1)]
        imged_reward = [] # rewards obtained from reward model [ r(s_0) : r(s_H-1) ]

        # start (parallel) rollout(s)
        for tau in range(len(imged_beliefs)):
            reward = self.reward_model.sample(imged_prior_states[tau], imged_beliefs[tau]) # [0 : H-1]
            state_value = self.target_critic_V( imged_prior_states[tau], imged_beliefs[tau] ) # [0 : H-1]
            imged_reward.append(reward)
            value_pred.append(state_value)

        value_pred = torch.stack(value_pred, dim=0)
        imged_reward = torch.stack(imged_reward, dim=0)

        # end of rollout
        # calculate lambda return

        lambda_returns = self.calculate_lambda_return_yuku(imged_reward, value_pred, value_pred[-1])  # [0 : H-2]

        # calculate loss_actor
        ...
        # start (parallel) rollout(s)
        for tau in range(len(imged_beliefs) - 1): # [0 : H-2]
            state_value = self.critic_V( imged_prior_states[tau].clone().detach(), imged_beliefs[tau].clone().detach() ) # [0 : H-2]
            value_pred.append(state_value)

        value_pred = torch.stack(value_pred, dim=0)

        critic_targets = lambda_returns.clone().detach()
        loss_critic = F.mse_loss(value_pred, critic_targets)
...
if __name__ == '__main__':
    ...
    # init Dreamer agent
    agent = Dreamer(o_dim, s_dim, a_dim, belief_dim, h_dim, max_action, sample_seq_len, imagination_horizon, df, replay_buffer_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, frac, tau, device)
...
# hyperparam dict
hyperparam_dict = {}
hyperparam_dict['env'] = 'MountainCarContinuous-v0'
hyperparam_dict['algo'] = 'dreamer5_dfexp19_yuku_STD1_targetCritic'
