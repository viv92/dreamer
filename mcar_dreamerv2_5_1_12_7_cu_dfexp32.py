### Program implementing DreamerV2 on MountainCar-v0 environment

# NOTE: discrete mountain car requires "optimism under uncertainty" to work - that is, all novel states should have high initial state value
# this can be achieved by setting the critic to yeild high value to all novel states
...
class StochasticNet_Gaussian(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc3_std(h).clip(-2, 20)
...
class Critic_V(nn.Module):
    def __init__(self, s_dim, belief_dim, h_dim):
        ...
        self.relu = nn.ReLU()
        self.init_state_value = 100 # for optimism under uncertainty
    ...
    def forward(self, state, belief):
        ...
        h = self.relu(self.fc2(h))
        h = self.relu(self.fc3(h))
        val = self.fc4(h) + self.init_state_value
        return val
...
class ReplayBuffer:
    def __init__(self, buf_size, seq_len, batch_size, o_dim, a_dim, max_ep_steps, frac, device):
        ...
        self.buf_reset_done = np.zeros((buf_size, 1))
        self.buf_learn_done = np.zeros((buf_size, 1))
        self.n_items = 0
        self.device = device
    ...
    def add(self, oar_tuple):
        observation, action, reward, reset_done, learn_done = oar_tuple
        ...
        self.buf_reset_done[index] = reset_done
        self.buf_learn_done[index] = learn_done
        self.n_items += 1
    ...
    def sample(self):
        ...
        while len(idx_list) < self.batch_size:
            ...
            if not (curr_idx in idx_chunk):
                 # ensure atleast 50% samples have a learn_done (premature termination states) - to encourage experiencing premature terminal states (required for learning df_model)
                if (1 in self.buf_learn_done) and ( len(idx_list) < (self.batch_size * self.frac) ):
                     if (1 in self.buf_learn_done[idx_chunk]):
        ...
        reset_done = torch.FloatTensor(self.buf_reset_done[idx]).to(self.device)
        learn_done = torch.FloatTensor(self.buf_learn_done[idx]).to(self.device)
        return (observation, action, reward, reset_done, learn_done)
...
class DreamerV2(nn.Module):
    ...
    def calculate_lambda_return(self, rewards, state_values_target, discounts):
        ...
        for t in range(len(rewards)-1, -1, -1): # t is just going from last element to first element, since all arrays are of length H-1
    ...
    def train(self):
        observation, action, reward, reset_done, learn_done = self.replay_buffer.sample() # sample = [o_t-1, a_t-1, r_t-1, d_t-1]
        ...
        for t in range(self.seq_len-1):
            ...
            prev_reset_done = reset_done[t]
            prev_learn_done = learn_done[t] # prev_done tells whether curr_state is terminal or not (also, this implies using df[t] * V(s_t) when discounting state values )

            # reset if curr_obs is from new episode (implied by prev_reset_done =  1)
            prev_state = prev_state * (1. - prev_reset_done)
            prev_action = prev_action * (1. - prev_reset_done)
            prev_belief = prev_belief * (1. - prev_reset_done)
            ...
            lp_reward = lp_reward * (1. - prev_reset_done) # since no reward if spawning into first state of an episode
        ...
        for tau in range(self.imagination_horizon):
            ...
            im_curr_action = self.get_action(im_curr_state.detach(), im_curr_belief.detach())
...
if __name__ == '__main__':
    ...
    for ep in range(init_random_episodes):
        ...
        while not done:
            ...
            if done:
                if ep_steps == env._max_episode_steps - 1:
                    # timeout termination
                    reset_done = 1
                    learn_done = 0
                    # add terminal experience to replay buffer
                    oar_tuple = [obs, action, reward, reset_done, learn_done]
                    ...
                    reset_done = 0
                    learn_done = 0
                    # add penultimate terminal experience to replay buffer
                    oar_tuple = [obs, action, reward, reset_done, learn_done]
                    agent.replay_buffer.add(oar_tuple)
                    ...
                    terminal_action = action * 0 # dummy value / action reset value (since no action for spawning into first state)
                    terminal_reward = reward * 0 # dummy reward (since no reward for spawning into first state)
                    # add terminal experience to replay buffer
                    oar_tuple = [terminal_obs, terminal_action, terminal_reward, reset_done, learn_done]
                    agent.replay_buffer.add(oar_tuple)
            else:
                # add intermediate experience to replay buffer
                reset_done = 0
                learn_done = 0
                oar_tuple = [obs, action, reward, reset_done, learn_done]
                agent.replay_buffer.add(oar_tuple)
    ...
    epsilon_schedule[:int(num_episodes * 0.8)] = np.linspace(1., explore_minLimit, int(num_episodes * 0.8))
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            if done:
                if ep_steps == env._max_episode_steps - 1:
                    # timeout termination
                    reset_done = 1 # set to 1 (rssm doesn't bother to learn about transition to first state after timeout - just resets)
                    learn_done = 0 # learning signal for df model after rssm resets to first state
                    # add terminal experience to replay buffer
                    oar_tuple = [observation, action_numpy, reward, reset_done, learn_done]
                    agent.replay_buffer.add(oar_tuple)
                else:
                    # premature termination
                    success_count += 1
                    print('!!!! success_count: ', success_count)
                    reset_done = 0 # no reset since next state will be the explicit terminal state
                    learn_done = 0 # set to 0 because terminal state should not be weighted by df=0, the next state (first state of next episode should be weighted by df=0 during imagination rollout)
                    # add penultimate terminal experience to replay buffer
                    oar_tuple = [observation, action_numpy, reward, reset_done, learn_done]
                    agent.replay_buffer.add(oar_tuple)
                    # explicit terminal state
                    reset_done = 0 # set to 0 so that rssm learns about transition to first state after premature termination
                    learn_done = 1 # learning signal for df model - set to 1 for so that rssm learns about premature termination. Also this encourages the next state (first state of next episode should be weighted by df=0 during imagination rollout)
                    terminal_observation = next_observation
                    terminal_action = action_numpy * 0 # dummy value / action reset value (since no action for spawning into first state)
                    terminal_reward = reward * 0 # dummy reward (since no reward for spawning into first state)
                    # add terminal experience to replay buffer
                    oar_tuple = [terminal_observation, terminal_action, terminal_reward, reset_done, learn_done]
                    agent.replay_buffer.add(oar_tuple)
            else:
                # add intermediate experience to replay buffer
                reset_done = 0
...
hyperparam_dict['algo'] = 'dreamerV2_5_1_12_7_cu_dfexp32_maxClip:20_'
...
ax[0,2].set_title('success_count:' + str(success_count))
ax[0,2].grid()
