...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        for t in range(self.seq_len-1):
            ...
            # reset if curr_obs is from new episode (implied by prev_prev_done =  1) - no reset for exp24
...
if __name__ == '__main__':
    ...
    # seed episodes
    seed_success = 0
    while not seed_success: # run random seed episodes until at least one successful run is encountered - mountain car specific
        obs = env.reset()
        done = False
        ep_steps = 0
        ...
        while not done:
            ...
            if done: # terminal transition
                ...
                if (ep_steps < env._max_episode_steps-1): # actual terminal state and not timeout termination
                    buf_done = 1 # so that frac % encourages sampling (and learning) of actual terminal states
                    seed_success = 1 # mountain car specific
                else: # timeout termination
                    buf_done = 0 # so that frac % doesn't end up picking timeout terminal states
        ...
        while not done:
            ...
            if done: # terminal transition
                ...
                if (ep_steps < env._max_episode_steps-1): # actual terminal state and not timeout termination
                    ...
                    buf_done = 0 # so that frac % doesn't end up picking timeout states
                # add terminal transition to replay buffer
...
hyperparam_dict['algo'] = 'dreamer5_dfexp24_HalfDetached'
