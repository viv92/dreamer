...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        imged_states = [torch.empty(0)] * self.imagination_horizon
        imged_beliefs = [torch.empty(0)] * self.imagination_horizon
        prior_means = [torch.empty(0)] * self.imagination_horizon
        prior_std_devs = [torch.empty(0)] * self.imagination_horizon
        imged_states[0] = im_curr_state
        imged_beliefs[0] = im_curr_belief

        # start (parallel) rollout(s)
        for tau in range(self.imagination_horizon-1):
            # im_curr_reward = self.reward_model.sample(im_curr_state, im_curr_belief)
            im_curr_action = self.get_action(imged_states[tau].clone().detach(), imged_beliefs[tau].clone().detach())
            # rollout step
            rssm_input = torch.cat((imged_states[tau], im_curr_action), dim=1)
            imged_states[tau+1], imged_beliefs[tau+1] = self.rssm.sample(imged_beliefs[tau], rssm_input)
            # prior_means[tau + 1], prior_std_devs[tau + 1], imged_beliefs[tau + 1] = self.rssm.forward(imged_beliefs[tau], rssm_input)
            # imged_states[tau + 1] = prior_means[tau + 1] + prior_std_devs[tau + 1] * torch.randn_like(prior_means[tau + 1])


        # end of rollout
        # calculate lambda return
        # state_values_target = torch.stack(state_values_target, dim=0)
        # rewards = torch.stack(rewards, dim=0)
        imged_states = torch.stack(imged_states, dim=0)[1:]
        imged_beliefs = torch.stack(imged_beliefs, dim=0)[1:]
        ...
        for tau in range(len(imged_beliefs)):
            reward = self.reward_model.sample(imged_states[tau], imged_beliefs[tau])
            state_value = self.critic_V( imged_states[tau], imged_beliefs[tau] )
        ...
        # start (parallel) rollout(s)
        for tau in range(len(imged_beliefs)):
            state_value = self.critic_V( imged_states[tau].clone().detach(), imged_beliefs[tau].clone().detach() )
            value_pred.append(state_value)
...
hyperparam_dict['algo'] = 'dreamer5_dfexp19_ablation3_1'
