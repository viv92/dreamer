...
class RSSM(nn.Module):
    ...
    def sample(self, prev_belief, x, o=None):
        ...
        # reshape logits from [b, classes*latents] to [b, classes, latents]
        logits_shape = logits.shape
        logits = logits.reshape(*logits_shape[:-1], s_classes, s_latents)
        ...
        # flatten sample 
        out = out.flatten(start_dim=-2, end_dim=-1)
        return out, belief
    ...
    def get_dist(self, prev_belief, x, o=None):
        ...
        dis = tdist.Independent(OneHotCategorical(logits=logits), 1)
        return dis
    ...
    def log_prob(self, prev_belief, x, y, o=None):
        ...
        # reshape y from [b, classes*latents] to [b, classes, latents]
        y_shape = y.shape
        y = y.reshape(*y_shape[:-1], s_classes, s_latents)
...
if __name__ == '__main__':
    ...
    s_latents = 32
    s_classes = 32
    s_dim = s_latents * s_classes
    belief_dim = 128 # 200
    lr_actor = 1 * (4e-5) # 4e-5
    lr_critic = 1 * (1e-4) # 1e-4
    lr_model = 5 * (2e-4) # 2e-4
    ...
    # minClip = -1 # -2
    STD = 0.1 # 1
    frac = 0.3 # fraction of sampled episodes containing terminal state transitions
    batch_size = 64 # 256 # 1024
    replay_buffer_size = 10**3
    num_episodes = 200 # 500
    random_seed = 1010
    record_episode = num_episodes // 5
    init_random_episodes = 10 # 10 # 50
    num_train_calls = 10 # 100
    train_episode = 1 # 10
    action_repeat = 2
    explore_minLimit = 0.1 # 0.
    max_time_steps = 500
    min_reward = -1 # 0
    max_reward = 0 # 1
    ...
    hyperparam_dict['algo'] = 'dreamerV2_6_10_df1_StateFix_noHistory_SB_frac'
            ...
            if done:
                ...
                while len(ep_trace) < (max_time_steps + 1):
                    ...
                    if done == 1: # terminated
                        terminal_reward = reward_normalized * 1 # terminal state is truly absorbing
                    else:
                        terminal_reward = reward_normalized * 1 # terminal state is truly absorbing
    ...
    epsilon_schedule[:int(num_episodes * 0.2)] = np.linspace(1., explore_minLimit, int(num_episodes * 0.2))
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        with torch.no_grad():
            ...
            while done == 0:
                ...
                ep_return += (df ** ep_steps) * (reward + 1)
                ep_steps += 1
