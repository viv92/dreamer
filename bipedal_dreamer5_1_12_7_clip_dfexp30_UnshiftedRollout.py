...
class RSSM(nn.Module):
    ...
    def forward(self, prev_belief, x, o=None):
        ...
        if o is None:
            ...
            logstd = self.q_fc2_std(h).clip(-2, 20)
            ...
            logstd = self.p_fc2_std(h).clip(-2, 20)
            std = torch.exp(logstd)
...
class StochasticNet_Base_Reward(nn.Module):
    def __init__(self, in_dim, h_dim, out_dim):
        super().__init__()
        self.fc1 = nn.Linear(in_dim, h_dim)
        self.fc2 = nn.Linear(h_dim, h_dim)
...
class StochasticNet_Base_Observation(nn.Module):
    def __init__(self, in_dim, h_dim, out_dim):
        super().__init__()
        self.fc1 = nn.Linear(in_dim, h_dim)
        self.fc2 = nn.Linear(h_dim, h_dim)
...
class Actor(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc5_std(h).clip(-2, 20)
        std = torch.exp(logstd)
        return mean, std
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        while len(idx_list) < self.batch_size:
            ...
            # start indices should be first step of episode (filtered using pt flag)
            idx_start_indices = np.where(self.buf_pt > 0)[0] + 1
            idx_start = np.random.choice(idx_start_indices)
            ...
            if not (curr_idx in idx_chunk):
                ...
                for j in range(len(idx_chunk)):
                    if self.buf_pt[idx_chunk[j]] == 2: # premature termination
                        fix_j = j
                        break
                if fix_j > -1:
                    idx_chunk[fix_j:] = idx_chunk[fix_j]

                # ensure atleast frac% samples have pt flag (so that samples include more of premature termination to encourage their learning)
                if len(idx_list) < (self.batch_size * self.frac):
                    if (2 in self.buf_pt[idx_chunk]):
...
class Dreamer(nn.Module):
    def __init__(self, o_dim, s_dim, a_dim, belief_dim, h_dim, max_action, seq_len, imagination_horizon, df, buf_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, frac, tau, device):
        ...
        self.reward_model = StochasticNet_Base_Reward(s_dim + belief_dim, h_dim, 1).to(device)
        self.observation_model = StochasticNet_Base_Observation(s_dim + belief_dim, h_dim, o_dim).to(device)
    ...
    def train(self):
        ...
        for t in range(self.seq_len-1):
            ...
            # reset if curr_obs is from new episode (implied by prev_done =  1)
        ...
        for tau in range(self.imagination_horizon):
            ...
            state_values_target.append( self.target_critic_V(im_next_state, im_next_belief).detach() )
...
if __name__ == '__main__':
    ...
    df = 1. # for bipedal since want to keep moving forward infinitely
    batch_size = 50
    num_episodes = 50
    random_seed = 0
        ...
        while not done:
            ...
            if done: # terminal transition
                ...
                if (ep_steps < env._max_episode_steps-1): # actual terminal state and not timeout termination
                    ...
                    seed_pt = 2 # premature termination (pt flag used for filtering start indices when sampling from replay buffer)
                    ...
                    buf_done = 1
                    seed_pt = 1 # timeout termination (pt flag used for filtering start indices when sampling from replay buffer)
                    # add terminal transition to replay buffer
                    oar_tuple = [obs, action, reward, buf_done]
                    agent.replay_buffer.add(oar_tuple, seed_pt)
        ...
        while not done:
            ...
            if done: # terminal transition
                ...
                if (ep_steps < env._max_episode_steps-1): # actual terminal state and not timeout termination
                    ...
                    buf_done = 0 # since rssm not to be reset
                    terminal_pt = 2 # premature termination (pt flag used for filtering start indices when sampling from replay buffer)
                    terminal_oar_tuple = [terminal_observation, terminal_action, terminal_reward, buf_done]
                    agent.replay_buffer.add(terminal_oar_tuple, terminal_pt)
                else: # timeout termination
                    success_count += 1
                    buf_done = 1
                    pt = 1 # timout termination (pt flag used for filtering start indices when sampling from replay buffer)
                    # add terminal transition to replay buffer
                    oar_tuple = [observation, action_numpy, reward, buf_done]
                    agent.replay_buffer.add(oar_tuple, pt)
            else: # intermediate transition
...
hyperparam_dict['algo'] = 'dreamer5_dfexp30_UnshiftedRollout'
...
hyperparam_dict['sample_seq_len'] = str(sample_seq_len)
