...
class RSSM(nn.Module):
    ...
    def sample(self, prev_belief, x, o=None):
        ...
        def rsample(logits):
            ...
            dis = tdist.independent.Independent(OneHotDist(logits=logits, unimix_ratio=0.01), 1)
...
class DreamerV2(nn.Module):
    ...
    def train(self):
        ...
        for tau in range(self.imagination_horizon):
            ...
            im_next_belief, im_next_state_prior, _, logits_prior, _ = self.rssm.sample(im_curr_belief, rssm_input)

            # intrinsic reward 
            if ep > delay_ep:
                logits_prior = logits_prior.reshape(logits_prior.shape[0], n_latents, n_classes)
                dis = tdist.independent.Independent(OneHotDist(logits=logits_prior, unimix_ratio=0.01), 1)
                prior_entropy = dis.entropy()
                # print('logits_prior.shape: ', logits_prior.shape)
                # print('dis.batch_shape: ', dis.batch_shape)
                # print('dis.event_shape: ', dis.event_shape)
                # print('prior_entropy.shape: ', prior_entropy.shape)
                im_reward_intrinsic = prior_entropy.unsqueeze(-1) * reward_intrinsic_scale
                im_reward_intrinsic_list.append(im_reward_intrinsic)
        ...
        if ep > delay_ep:
            im_rewards = torch.stack(im_reward_intrinsic_list, dim=0) # [t+1 : t+1+H]
        ...
        lambda_returns_intrinsic = 0
        if ep > delay_ep:
            lambda_returns_intrinsic = self.calculate_lambda_return(im_rewards + rewards, discounts, state_values).detach()
        ...
        advantage = (lambda_returns_intrinsic - state_values[:-1]) / ret_scale # advantage[t+1 : t+H]
...
if __name__ == '__main__':
    ...
    rho = 1 # 0.75 # 0.25 # 0 # 1 # used for weighing actor dynamics loss and actor reinforce loss
    eta = 3e-4 # used for weighing entropy regulaization in actor loss
    df = 0.997
    frac = 0 # 0.25
    reward_intrinsic_scale = 1e-1
    delay_ep = -1 # 50
    terminal_reward = 0
    # STD = 1
    minClip, maxClip = -2, 2
    ...
    num_episodes = 400 # 100 # 200 # 400
    init_random_episodes = 0
    num_train_calls = 30 # 10 # 50 
    train_episode = 5 # 1
    ...
    hyperparam_dict['algo'] = 'dreamerV3_r2_ICMprior_rssmNorms_DistIndep_Hsum_STnotGumbel'
    ...
    # hyperparam_dict['terminalR'] = str(terminal_reward)
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            belief, _, state, logits_prior, logits_posterior = agent.rssm.sample(prev_belief, rssm_input, obs_input)
            ...
            if terminated:
                reward_extrinsic += terminal_reward
            ...
            # calculate intrinsic reward - ICM prior 
            reward_intrinsic = 0
            if ep > delay_ep:
                logits_prior = logits_prior.reshape(logits_prior.shape[0], n_latents, n_classes)
                dis = tdist.independent.Independent(OneHotDist(logits=logits_prior, unimix_ratio=0.01), 1)
                ...
                # print('dis.event_shape: ', dis.event_shape)
                # print('prior_entropy.shape: ', prior_entropy.shape)
                reward_intrinsic = prior_entropy.mean().item() * reward_intrinsic_scale
...
ax[0,0].plot(ep_returns_moving_mean, color='blue', label='ep_return_intrinsic')
ax[0,0].plot(ep_returns_extrinsic_moving_mean, color='green', label='ep_return_extrinsic')
ax[0,0].legend()
ax[0,0].set_title('return_intrinsic:{:.2f} return_extrinsic:{:.2f}'.format(ep_returns_moving_mean[-1], ep_returns_extrinsic_moving_mean[-1]))
