    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            if done:
                if ep_steps == env._max_episode_steps - 1:
                    # timeout termination
                    reset_done = 0 # set to 0 so that rssm learns about transition to first state after timeout
                    learn_done = 1 # learning signal for df model - set to 1 for so that rssm learns about timeout. Also this encourages the next state (first state of next episode should be weighted by df=0 during imagination rollout)
                    # add terminal experience to replay buffer
...
hyperparam_dict['algo'] = 'dreamerV2_5_1_12_7_cu_dfexp31_maxClip:20_'
