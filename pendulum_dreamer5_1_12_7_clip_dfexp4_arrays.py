...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        # init belief and state for latent imagination / rollouts
        # flatten time and batch dimension of into one - for parallel rollouts
        im_init_belief = torch.flatten(belief_list, start_dim=0, end_dim=1)
        im_init_state = torch.flatten(state_list, start_dim=0, end_dim=1)

        # containers to keep required values from imagination rollouts
        im_belief_list = [im_init_belief] # list to store all beleifs - to avoid overwriting over the same variable 'im_curr_belief'
        im_state_list = [im_init_state] # list to store all states - to avoid overwriting over the same variable 'im_curr_states'
        ...
        for tau in range(self.imagination_horizon):
            im_curr_reward = self.reward_model.sample(im_state_list[tau], im_belief_list[tau])
            im_curr_action = self.get_action(im_state_list[tau], im_belief_list[tau])
            # rollout step
            rssm_input = torch.cat((im_state_list[tau], im_curr_action), dim=1)
            im_next_state, im_next_belief = self.rssm.sample(im_belief_list[tau], rssm_input)
            # store required values
            state_values.append(self.critic_V( im_state_list[tau].clone().detach(), im_belief_list[tau].clone().detach() ))
...
hyperparam_dict['algo'] = 'dreamer5_1_12_7_clip_dfexp4_arrays'
