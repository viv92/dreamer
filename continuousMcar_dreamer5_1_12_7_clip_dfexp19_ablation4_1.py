...
class Dreamer(nn.Module):
    ...
    def calculate_lambda_return(self, rewards, state_values_target):
        """
        # rewards obtained from reward model [ r(s_1) : r(s_H) ]
        # state values obtained from target_critic [ v(s_2) : v(s_H) ]
        """
        lambda_returns = []
        accumulator = state_values_target[-1]
        for t in range(len(state_values_target)-1, -1, -1):
    ...
    def train(self):
        ...
        lambda_returns = self.calculate_lambda_return(imged_reward, value_pred[1:])
...
hyperparam_dict['algo'] = 'dreamer5_dfexp19_ablation4_1'
