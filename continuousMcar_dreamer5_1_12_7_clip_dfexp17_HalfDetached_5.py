...
class StochasticNet_Base(nn.Module):
    def __init__(self, in_dim, h_dim, out_dim):
        ...
        self.fc3_0_mean = nn.Linear(h_dim, h_dim)
        self.fc3_mean = nn.Linear(h_dim, out_dim)
        self.fc3_0_std = nn.Linear(h_dim, h_dim)
    ...
    def forward(self, state, belief):
        ...
        h_mean = self.relu(self.fc3_0_mean(h))
        mean = self.fc3_mean(h_mean)
        h_std = self.relu(self.fc3_0_std(h))
        logstd = self.fc3_std(h_std).clip(-2, 2)
        std = torch.exp(logstd)
        # std = 1.
        return mean, std
...
if __name__ == '__main__':
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        if ep % (num_episodes//10) == 0:
            print('ep:{} \t ep_return:{:.3f}'.format(ep, ep_return))

        # print successful return
        # if ep_steps < env._max_episode_steps - 1:
        #     print('!!!success ep:{} \t ep_return:{:.3f} \t reward:{:.3f} \t success_count:{}'.format(ep, ep_return, reward, success_count))
        # else:
        #     print('ep:{} \t ep_return:{:.3f}'.format(ep, ep_return))
...
hyperparam_dict['algo'] = 'dreamer5_dfexp17_HalfDetached_5'
