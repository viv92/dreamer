...
class Actor(nn.Module):
    ...
    def get_policy_logprob(self, state, belief):
        ...
        return true_policy_logprob

    def get_policy_entropy(self, state, belief):
        mean, std = self.forward(state, belief)
        z = tdist.Normal(0, 1).sample()
        gaussian_action = z * std + mean
        # true_action = self.tanh(gaussian_action) * self.max_action
        gaussian_policy = tdist.Normal(mean, std)
        policy_entropy = gaussian_policy.entropy()
...
class Dreamer(nn.Module):
    def __init__(self, o_dim, s_dim, a_dim, belief_dim, h_dim, max_action, seq_len, imagination_horizon, df, buf_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, rho, eta, frac, tau, device):
        super().__init__()
    ...
    def train(self, explore_std):
        ...
        for tau in range(self.imagination_horizon):
            ...
            im_curr_action = self.get_action(im_curr_state, im_curr_belief, explore_std)
            im_curr_log_pi = self.actor.get_policy_logprob( im_curr_state.clone().detach(), im_curr_belief.clone().detach() )
            # im_curr_log_pi = im_curr_pi.log_prob(torch.round(im_curr_action.detach()))
            im_curr_entropy_pi = self.actor.get_policy_entropy( im_curr_state.clone().detach(), im_curr_belief.clone().detach() )
            # rollout step
            rssm_input = torch.cat((im_curr_state, im_curr_action), dim=1)
            im_next_state, im_next_belief = self.rssm.sample(im_curr_belief, rssm_input)
        ...
        loss_actor_dynamics = -lambda_returns
...
if __name__ == '__main__':
    ...
    # init Dreamer agent
    agent = Dreamer(o_dim, s_dim, a_dim, belief_dim, h_dim, max_action, sample_seq_len, imagination_horizon, df, replay_buffer_size, batch_size, lr_actor, lr_critic, lr_model, vib_beta, _lambda, alpha, rho, eta, frac, tau, device)
    ...
    explore_std_schedule = np.ones(num_episodes) * 0.05
    explore_std_schedule[:int(num_episodes * 0.8)] = np.linspace(explore_std_init, 0.05, int(num_episodes * 0.8))
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            # if render_final_episodes and (ep % 5 == 0):
                env.render()
...
hyperparam_dict['algo'] = 'dreamer5_1_12_7_clip_dfexp4'
