...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        for tau in range(len(imged_beliefs)):
            reward = self.reward_model.sample(imged_prior_states[tau], imged_beliefs[tau])
            state_value = self.critic_V( imged_prior_states[tau], imged_beliefs[tau] )
        ...
        # start (parallel) rollout(s)
        for tau in range(len(imged_beliefs)):
            state_value = self.critic_V( imged_prior_states[tau].clone().detach(), imged_beliefs[tau].clone().detach() )
            value_pred.append(state_value)
...
hyperparam_dict['algo'] = 'dreamer5_dfexp19_yuku_STD1'
