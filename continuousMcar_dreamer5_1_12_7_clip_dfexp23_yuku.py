...
class RSSM(nn.Module):
    ...
    def forward(self, prev_belief, x, o=None):
        ...
        if o is None:
            ...
            std = F.softplus(self.q_fc2_std(h)) + self.min_std
        else:
            ...
            # std = torch.exp(logstd)
            std = F.softplus(self.p_fc2_std(h)) + self.min_std
...
class Actor(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        raw_init_std = torch.ones_like(std) * self._init_std
...
class Dreamer(nn.Module):
    ...
    def calculate_lambda_return_yuku(self, imged_reward, value_pred, bootstrap, discount=0.99, lambda_=0.95):
        ...
        imged_reward = imged_reward[1:] # [1 : H-1]
    ...
    def train(self):
        ...
        for t in range(len(beliefs)):
            ...
            # log prob q(r_t | s_t)
            lp_reward = self.reward_model.log_prob(curr_state, curr_belief, reward[t])
            lp_reward = lp_reward * nonterminals[t] # skip reward loss for first state (since no reward for spawning into the first state)
        ...
        for tau in range(len(imged_beliefs)):
            ...
            state_value = self.critic_V( imged_prior_states[tau], imged_beliefs[tau] ) # [0 : H-1]
        ...
        value_pred = []
        value_states = imged_prior_states.detach()
        value_beliefs = imged_beliefs.detach()

        # start (parallel) rollout(s)
        for tau in range(len(imged_beliefs)): # [0 : H-1]
            state_value = self.critic_V( value_states[tau], value_beliefs[tau] ) # [0 : H-1]
            value_pred.append(state_value)

        value_pred = torch.stack(value_pred, dim=0)[:-1] # [0 : H-2]

        critic_targets = lambda_returns.clone().detach() # [0 : H-2 ]
...
hyperparam_dict['algo'] = 'dreamer5_dfexp23_yuku'
