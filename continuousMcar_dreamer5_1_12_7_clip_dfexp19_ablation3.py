...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        imged_states = [im_curr_state]
        imged_beliefs = [im_curr_belief]
        ...
        for tau in range(self.imagination_horizon):
            ...
            state_values_target.append( self.critic_V(im_curr_state, im_curr_belief) )
        ...
        lambda_returns = self.calculate_lambda_return_yuku(rewards, state_values_target, state_values_target[-1])  # [0 : H-2]
        ...
        # start (parallel) rollout(s)
        for tau in range(self.imagination_horizon):
            im_curr_state, im_curr_belief = imged_states[tau], imged_beliefs[tau]
            # store required values
            state_values.append(self.critic_V( im_curr_state.clone().detach(), im_curr_belief.clone().detach() ))


        state_values = torch.stack(state_values, dim=0) # [0 : H-2]
...
hyperparam_dict['algo'] = 'dreamer5_dfexp19_ablation3'
