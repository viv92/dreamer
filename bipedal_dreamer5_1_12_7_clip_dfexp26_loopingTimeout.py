...
class ReplayBuffer:
    def __init__(self, buf_size, seq_len, batch_size, o_dim, a_dim, frac, device):
        ...
        self.buf_done = np.zeros((buf_size, 1)) # termination (any - timeout or premature)
    ...
    def sample(self):
        ...
        while len(idx_list) < self.batch_size:
            ...
            if not (curr_idx in idx_chunk):
                ...
                # if done == 1 for an index in idx_chunk, repeat that index for rest of the chunk
                fix_j = -1
                for j in range(len(idx_chunk)):
                    if self.buf_done[idx_chunk[j]] == 1:
                        fix_j = j
                        break
                if fix_j > -1:
                    if self.buf_pt[fix_j] == 1: # premature terminal state
                        # fill repeat the last step for rest of the chunk indices
                        idx_chunk[fix_j:] = idx_chunk[fix_j]
                    else: # timeout terminal state
                        # looping repeat the remaining indices of the chunk
                        remaining_len = len(idx_chunk) - fix_j - 1
                        remaining_idx = np.zeros(remaining_len)
                        k = fix_j
                        for i in range(remaining_len):
                            remaining_idx[i] = idx_chunk[k]
                            k -= 1
                            if k < 0:
                                k = fix_j
                        remaining_idx = np.flip(remaining_idx)
                        idx_chunk[fix_j+1:] = remaining_idx
...
class Dreamer(nn.Module):
    ...
    def train(self):
        ...
        for t in range(self.seq_len-1):
            ...
            # reset if curr_obs is from new episode  - no reseting needed for exp 26
...
if __name__ == '__main__':
    ...
    for ep in range(init_random_episodes):
        ...
        while not done:
            ...
            # obs_cat_epsteps = np.concatenate((obs, [ep_steps]), axis=0)

            action = env.action_space.sample()
            next_obs, reward, done, _ = env.step(action)

            # add experience to repaly buffer - along with handling terminal state

            seed_pt = 0 # timeout termination
            if done: # terminal transition
                ...
                if (ep_steps < env._max_episode_steps-1):
                    ...
                    success_count += 1 # mountain car specific
                # add terminal transition to replay buffer
                oar_tuple = [obs, action, reward, 0]
                ...
                terminal_done = 1
                oar_tuple = [terminal_obs, terminal_action, terminal_reward, terminal_done]
                agent.replay_buffer.add(oar_tuple, seed_pt)
            else:
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            # append ep_steps as part of observation input
            # obs_cat_epsteps = np.concatenate((observation, [ep_steps]), axis=0)
            obs_input = torch.FloatTensor(observation).unsqueeze(0)
            ...
            # next_obs_cat_epsteps = np.concatenate((next_observation, [ep_steps+1]), axis=0)


            # adding experience to repaly buffer - along with handling episode termination

            pt = 0 # timeout termination
            if done: # terminal transition
                # differentiate between actual_terminal_state and timeout_terminal_state
                if (ep_steps < env._max_episode_steps-1):
                    pt = 1 # premature termination
                else:
                    success_count += 1 # mountain car specific
                # add terminal transition to replay buffer
                oar_tuple = [observation, action_numpy, reward, 0]
                agent.replay_buffer.add(oar_tuple, 0)
                # add explicit terminal state
                terminal_obs = next_observation
...
hyperparam_dict['algo'] = 'dreamer5_dfexp26_loopingTimeout'
