...
# This implementation is same as dreamerv2_5_1_12_7_cu.py but without the explicit terminal state ( so skipping is added for the reward and df loss at the first step of a new episode)
...
class DreamerV2(nn.Module):
    ...
    def train(self):
        ...
        for t in range(self.seq_len-1):
            ...
            # log prob q(r_t | s_t)
            lp_reward = self.reward_model.log_prob(curr_state, curr_belief, prev_reward) * (1. - prev_prev_done)
            ...
            lp_df = self.df_model.log_prob(curr_state, curr_belief, df_target) * (1. - prev_prev_done)
