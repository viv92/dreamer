...
class RSSM(nn.Module):
    ...
    def forward(self, prev_belief, x, o=None):
        ...
        if o is None:
            ...
            logstd = self.q_fc2_std(h).clip(minClip, 2)
            ...
            logstd = self.p_fc2_std(h).clip(minClip, 2)
            std = torch.exp(logstd)
...
class StochasticNet_Base(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc2_std(h).clip(minClip, 2)
        std = torch.exp(logstd)
        # std = 1.
        return mean, std
...
if __name__ == '__main__':
    ...
    imagination_horizon = 8 # 16 # length of imagined rollouts using the learnt dynamics model (when behaviour learning)
    ...
    train_episode = 1 # 25
    random_seed = 1010
    init_random_episodes = 64 # 5
    num_train_calls = 10 # 1 # 50
    ...
    hyperparam_dict['algo'] = 'dreamer6_10_dynTimeSamp_noHistory_SB_NormRew'
    ...
    for ep in range(init_random_episodes):
        ...
        while done == 0:
            action_discrete = env.action_space.sample()
            ...
            # convert discrete action in {0,1} to float in [-1,1]
            action = action_discrete * 2 - 1
            action = [action]
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        with torch.no_grad():
            ...
            while done == 0:
                ...
                # convert action from float in [-1,1] to discrete in {0,1}
                action_discrete = int(action_numpy[0] > 0)

                next_observation, reward, terminated, truncated, _ = env.step(action_discrete)
                if terminated:
                    done = 1
                if truncated:
                    done = 2
