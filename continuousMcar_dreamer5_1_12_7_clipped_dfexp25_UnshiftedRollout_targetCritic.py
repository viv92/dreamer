...
class RSSM(nn.Module):
    ...
    def forward(self, prev_belief, x, o=None):
        ...
        if o is None:
            ...
            logstd = self.q_fc2_std(h).clip(-2, 8)
            ...
            logstd = self.p_fc2_std(h).clip(-2, 8)
            std = torch.exp(logstd)
...
class StochasticNet_Base(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc3_std(h).clip(-2, 8)
        std = torch.exp(logstd)
        # std = 1.
        return mean, std
...
class Actor(nn.Module):
    ...
    def forward(self, state, belief):
        ...
        logstd = self.fc5_std(h).clip(-2, 8)
        std = torch.exp(logstd)
        return mean, std
...
if __name__ == '__main__':
    ...
    for ep in tqdm(range(num_episodes)):
        ...
        while not done:
            if render_final_episodes and ( (ep % 10 == 0) or (ep > (num_episodes - 5)) ):
...
hyperparam_dict['algo'] = 'dreamer5_dfexp25_UnshiftedRollout_clipped8_'
